...
#awk -F "$" '{print $1, $7}' ${PATH_DATA}/asthma_diagnosis_gp_script.txt | \
#    grep -w -F -f ${PATH_OUTPUT}/eid_excl_ecb_asthma_diagnosis.txt | \
#    cut -d " " -f 2- | \
#    sed s/'^[*]\|^["]'//g | sed s/'^[*]'//g | sort -u -f \
#    grep -w -F -f - ${PATH_DATA}/Asthma_Meds.txt | \
#    sort -u > ${PATH_DATA}/Asthma_Meds_shared_excl_ecb_asthma_diagnosis.txt
...
#    sort -u > ${PATH_DATA}/Asthma_Meds_notshared_excl_ecb_asthma_diagnosis.txt
...
#    sed s/'^[*]\|^["]'//g | sed s/'^[*]'//g | sort -u -f |\
#    grep -w -F -f - ${PATH_DATA}/Asthma_Meds.txt | \
#    sort -u > ${PATH_DATA}/Asthma_Meds_shared_excl_QCRPecb_asthma_diagnosis.txt

sed 's/\$/ /g' ${PATH_DATA}/asthma_diagnosis_gp_script.txt | \
    grep -w -F -f ${PATH_DATA}/eid_excl_QCRPecb_asthma_diagnosis.txt | \
    grep -F -f ${PATH_DATA}/Asthma_Meds_shared_excl_QCRPecb_asthma_diagnosis.txt \
    > ${PATH_DATA}/noRPecb_asthma_diagnosis_gp_script.txt

awk '{print $1}' ${PATH_DATA}/noRPecb_asthma_diagnosis_gp_script.txt | sort -u \
    > ${PATH_DATA}/Eid_noRPecb_asthma_diagnosis_gp_script.txt
...
#    sed s/'^[*]\|^["]'//g | sed s/'^[*]'//g | sort -u -f \
#    grep -v -w -F -f - ${PATH_DATA}/Asthma_Meds.txt | \
#    sort -u > ${PATH_DATA}/Asthma_Meds_notshared_excl_QCRPecb_asthma_diagnosis.txt
